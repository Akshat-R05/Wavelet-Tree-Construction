...
//Submitted By - Akshat Rohil(2K20/IT/13)
...
wavelet_tree::wavelet_tree(int* start, int* end, int min, int max){  // function to make wavelet tree
    ...
    this->min = min;  // initializing the values
        ...
        if (start >= end){  // in case the array is NULL
            ...
            freq.reserve(end - start + 1);
 
            freq.push_back(0);                   
 
            for (auto it = start; it != end; it++) // tree will not have any child since data is homogeneous
        ...
        freq.reserve(end - start + 1); // reserving space 
